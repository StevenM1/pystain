...
subject_ids = [13054, 13061, 13072, 13074, 13095, 14037, 14051, 14069, 15033, 15035, 15052, 15055]
#subject_ids = [15055]
...
for subject_id in subject_ids:
    for fwhm in [0.15, 0.3]:
        ...
        try:
            ...
            fn = '/home/gdholla1/data/post_mortem/clusterings_v1/%s_%s.pdf' % (subject_id, fwhm)
            pdf = PdfPages(fn)
            ...
            for n_components in xrange(1, max_n_components+1):
                ...
                for i in xrange(1, n_components+1):
                    ...
                    for j, q in enumerate([0.25, 0.5, 0.75]):
                        ...
                        slice_axial = cluster.get_proportional_slice(1-q, 'axial')
                        slice_sagittal = cluster.get_proportional_slice(q, 'sagittal')  

                        plt.subplot(3,3,(3*j)+1)
                        cluster.plot_cluster_probability(component=i, n_components=n_components,slice=slice_coronal, mask_thr=1e-12)

                        plt.subplot(3,3,(3*j)+2)
                        cluster.plot_cluster_probability('axial', component=i, n_components=n_components,slice=slice_axial, mask_thr=1e-12)        

                        plt.subplot(3,3,(3*j)+3)
                        cluster.plot_cluster_probability('sagittal', component=i, n_components=n_components,slice=slice_sagittal, mask_thr=1e-12)                
