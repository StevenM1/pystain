...
def plot_trace_tri_complex(trace, dataset, stain, **kwargs):
    ...
    pdf3 = trace['w'][2] * exgauss_pdf(t, trace['mu'][2], trace['sigma'][2], trace['nu'][2])
    ...
    l2 = plt.plot(t, pdf3, color=sns.color_palette()[2], label='cluster 2', **kwargs)
    l3= plt.plot(t, pdf1 + pdf2 + pdf3, color=sns.color_palette()[3], label='cluster 1 + 2 + 3', **kwargs)
...
for subject_id in subject_ids[1:2]:
    ...
    pdf_fn = os.path.join(d, 'clusters_tri_complex.pdf')
    pdf = PdfPages(pdf_fn)
    ...
    for stain in dataset.stains[:]:
        ...
        with pm.Model() as tri_complex_model:   

            w = pm.Dirichlet('w', np.ones(3) * 10)

            nu = pm.HalfNormal('nu', tau=1/25., shape=3)
            mu = pm.Normal('mu', mu=0, tau=1/25., shape=3)
            sigma = pm.HalfNormal('sigma', tau=1./10., shape=3)

            expon = pm.Mixture('mix', w,
                               [ExGaussian.dist(mu=mu[0], sigma=sigma[0], nu=nu[0]),
                                ExGaussian.dist(mu=mu[1], sigma=sigma[1], nu=nu[1]),
                                ExGaussian.dist(mu=mu[2], sigma=sigma[2], nu=nu[2])], observed=x)

            order_means_potential = pm.Potential('order_means_potential',
                                             tt.switch(nu[1]-nu[0] < 0, -np.inf, 0) + tt.switch(nu[2]-nu[1] < 0, -np.inf, 0))

            
            trace_fn = os.path.join(d, '{subject_id}_{stain}_tri_complex'.format(**locals()))
    ...
    #         start = pm.find_MAP()
            ...
            try:
                plt.figure()
                pm.traceplot(trace)
                plt.suptitle('{subject_id} - {stain} (three clusters, complex)'.format(**locals()))
                ...
                for i in np.arange(0, trace_size, trace_size/500):
                    plot_trace_tri_complex(trace[i], dataset, stain, alpha=0.1, lw=3)

                sns.despine()
                
                plt.title('{subject_id} - {stain} (three clusters, complex)'.format(**locals()))
                plt.savefig(pdf, format='pdf')
                plt.close(plt.gcf())
            ...
            results_pandas.to_pickle(os.path.join(d, 'results_three_complex.pandas'))
