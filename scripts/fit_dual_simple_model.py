...
def plot_trace_dual(trace, dataset, stain, **kwargs):
...
for subject_id in subject_ids[1:2]:
    ...
    pdf_fn = os.path.join(d, 'clusters_dual.pdf')
    pdf = PdfPages(pdf_fn)
    ...
    for stain in dataset.stains:
        ...
        with pm.Model() as dual_model:   

            w = pm.Dirichlet('w', np.ones(2) * 10)

            nu = pm.HalfNormal('nu', tau=5., shape=2)
            ...
            expon = pm.Mixture('mix', w,
                               [ExGaussian.dist(mu=mu, sigma=sigma, nu=nu[0]),
                                ExGaussian.dist(mu=mu, sigma=sigma, nu=nu[1])], observed=x)

            order_means_potential = pm.Potential('order_means_potential',
                                             tt.switch(nu[1]-nu[0] < 0, -np.inf, 0))

            
            trace_fn = os.path.join(d, '{subject_id}_{stain}_dual'.format(**locals()))
    ...
    #         start = pm.find_MAP()
            ...
            try:
                plt.figure()
                pm.traceplot(trace)
                plt.suptitle('{subject_id} - {stain} (two clusters, simple)'.format(**locals()))
                ...
                for i in np.arange(0, trace_size, trace_size/500):
                    plot_trace_dual(trace[i], dataset, stain, alpha=0.1, lw=3)

                sns.despine()
                
                plt.title('{subject_id} - {stain} (two clusters, simple)'.format(**locals()))
                plt.savefig(pdf, format='pdf')
    ...
    results.to_pickle(os.path.join(d, 'results_dual.pandas'))
