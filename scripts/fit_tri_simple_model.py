...
def plot_trace_tri(trace, dataset, stain, **kwargs):
    
    t = np.linspace(0, dataset.vmax[dataset._get_index_stain(stain)], 1000)
    
    pdf1 = trace['w'][0] * exgauss_pdf(t, trace['mu'], trace['sigma'], trace['nu'][0])
    pdf2 = trace['w'][1] * exgauss_pdf(t, trace['mu'], trace['sigma'], trace['nu'][1])
    pdf3 = trace['w'][2] * exgauss_pdf(t, trace['mu'], trace['sigma'], trace['nu'][2])
    ...
    l3 = plt.plot(t, pdf3, color=sns.color_palette()[2], label='cluster 3', **kwargs)
    l4= plt.plot(t, pdf1 + pdf2 + pdf3, color=sns.color_palette()[3], label='cluster 1 + 2 + 3', **kwargs)
...
for subject_id in subject_ids[1:2]:
    ...
    pdf_fn = os.path.join(d, 'clusters_tri.pdf')
    pdf = PdfPages(pdf_fn)
    ...
    for stain in dataset.stains:
        ...
        with pm.Model() as tri_model:   

            w = pm.Dirichlet('w', np.ones(3) * 10)

            nu = pm.HalfNormal('nu', tau=5., shape=3)
            mu = pm.Normal('mu', mu=0, tau=1/10.)
            sigma = pm.HalfNormal('sigma', tau=1./10.)

            expon = pm.Mixture('mix', w,
                               [ExGaussian.dist(mu=mu, sigma=sigma, nu=nu[0]),
                                ExGaussian.dist(mu=mu, sigma=sigma, nu=nu[1]),
                                ExGaussian.dist(mu=mu, sigma=sigma, nu=nu[2]) ], observed=x)

            order_means_potential = pm.Potential('order_means_potential',
                                             tt.switch(nu[1]-nu[0] < 0, -np.inf, 0) + tt.switch(nu[2]-nu[1] < 0, -np.inf, 0))

            
            trace_fn = os.path.join(d, '{subject_id}_{stain}_tri'.format(**locals()))
    ...
    #         start = pm.find_MAP()
            ...
            try:
                plt.figure()
                pm.traceplot(trace)
                plt.suptitle('{subject_id} - {stain} (three clusters, simple)'.format(**locals()))
                ...
                for i in np.arange(0, trace_size, trace_size / 500):
                    plot_trace_tri(trace[i], dataset, stain, alpha=0.1, lw=3)

                sns.despine()
                
                plt.title('{subject_id} - {stain} (three clusters, simple)'.format(**locals()))
                plt.savefig(pdf, format='pdf')
            ...
            results_pandas.to_pickle(os.path.join(d, 'results_three_simple.pandas'))
